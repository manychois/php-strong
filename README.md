...
| PSR-15 Handlers & Middleware | `Manychois\PhpStrong\Http` | `MiddlewarePipeline`, a request handler dispatching middleware in order to a fallback handler, with optional lazy resolution of middleware service ids from a PSR-11 container; plus `Cookie`, `CookieBag` and `CookieStore`/`CookieAwareClient` for reading, queuing and remembering cookies on either side of a request. | [docs/http.md](docs/http.md) |
...
| `Manychois\PhpStrong\Http` | `NativeSession`, a lazily started `$_SESSION` wrapper whose `SessionInterface` extends `DataReaderInterface`, so session data is read with the same strongly typed, dot-notation accessors. | [docs/http.md](docs/http.md) |
